...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (empty($given_name_err) && empty($surname_err) && empty($sex_err) && empty($address_err) && empty($email_err) && empty($contact_number_err)) {
    $sql = "UPDATE `accounts` SET given_name = ?, surname = ?, sex = ?, address = ?, email = ?, contact_number = ?, section_id = ? WHERE `id` = '$accountId'";

    $section = trim($_POST['section']);

    if ($stmt = mysqli_prepare($connection, $sql)) {
      mysqli_stmt_bind_param($stmt, "ssssssi", $param_given_name, $param_surname, $param_sex, $param_address, $param_email, $param_contact_number, $param_section);
          ...
          <div class="shadow-lg card-body p-5 text-left" style="border-radius: 1rem;">
            <form autocomplete="off" method="POST">
              ...
              <div class="form-group">
                ...
                <?php
                while (($student = $accCheckResult) && ($section = mysqli_fetch_assoc($sectionCheckQuery))) {
                  $sectionId = $section['id'];
                  $sectionName = $section['year_level'] . ' - ' . $section['name'];

                  if ($sectionId == $student['section_id']) {
                    echo "<option selected value='$sectionId'>$sectionName</option>";
                  } else {
