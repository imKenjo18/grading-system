              ...
              <div class="card-body text-center" style="border-radius: 1rem;">
                <table class="table table-striped table-xxl table-hover mb-0">
                  ...
                  <tbody>
                    ...
                    while ($section = mysqli_fetch_assoc($sectionQuery)) {
                      $id = $section['id'];
                      $section_name = $section['name'];
                      $year_level = $section['year_level'];
                      $adviser_id = $section['adviser_id'];
                      
                      $teacherCheck = "SELECT *  FROM `accounts` WHERE `privilege` = 'teacher' AND `id` = '$adviser_id'";
                      $teacherSQL = mysqli_query($connection, $teacherCheck);
                      $teacher = mysqli_fetch_assoc($teacherSQL);

                      if (empty($teacher)) {
                        $adviser = '';
                      } else {
                        $adviser = $teacher['full_name'];
                      }

                      echo '<tr><td>' . $section_name . '</td><td>' . $year_level . '</td><td>' . $adviser . '</td><td><a href="view-section?id=' . $id .'" title="View Section"><img class="icons" id="view-class-svg" src="assets/view-class.svg" width="25" height="25" alt="View Section"></a><a href="edit-section?id=' . $id .'" title="Edit"><img class="icons" id="edit-svg" src="assets/edit.svg" width="25" height="25" alt="Edit"></a><a onClick="return confirm(\'Proceed to Delete?\');" href="delete-section?id=' . $id . '" title="Delete"><img class="icons" id="delete-svg" src="assets/delete.svg" width="28" height="28" alt="Delete"></a></td></tr>';
                    }
                    ?>
                  </tbody>
                </table>
